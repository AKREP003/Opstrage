...
def findminterms(
        a):  # Function for finding out which minterms are merged. For example, 10-1 is obtained by merging 9(1001) and 11(1011)
    gaps = a.count('-')
    if gaps == 0:
        return [str(int(a, 2))]
...
def gr(mt, dc):
    mt = [int(i) for i in mt.strip().split()]
...
if __name__ == "__main__":
    mt = input("Enter the minterms: ")
    dc = input("Enter the don't cares(If any): ")
